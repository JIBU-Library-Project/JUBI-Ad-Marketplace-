...
function AllAdsPage() {
  ...
  useEffect(() => {
    setAds(dummyAds); // Simulate API call
  }, []);

  // Filter logic
  const filteredAds = ads.filter((ad) => {
    const matchesQuery = ad.title.toLowerCase().includes(query.toLowerCase());
    const matchesCategory = activeCategory ? ad.category === activeCategory : true;
        ...
        <div className="md:w-2/3 w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {filteredAds.length === 0 ? (
              <p className="text-gray-500 col-span-2">No ads match your search.</p>
            ) : (
